...
/**
 * @file
 * Contains Drupal\search_api\Plugin\search_api\datasource\EntityDatasourceController.
 */
 ...
 */
class EntityDatasource extends DatasourcePluginBase {
  ...
  public function getIdFieldInfo() {
    ...
    if (empty($info['entity keys']['id'])) {
      throw new DatasourceException(t("Entity type @type doesn't specify an ID key.", array('@type' => $info['label'])));
    }
    $field = $info['entity keys']['id'];
    if (empty($properties['properties'][$field]['type'])) {
      throw new DatasourceException(t("Entity type @type doesn't specify a type for the @prop property.", array('@type' => $info['label'], '@prop' => $field)));
    }
    $type = $properties['properties'][$field]['type'];
    if (search_api_is_list_type($type)) {
      throw new DatasourceException(t("Entity type @type uses list field @prop as its ID.", array('@type' => $info['label'], '@prop' => $field)));
    }
    if ($type == 'token') {
      $type = 'string';
    }
